...
try:
    ...
    print("Please install required libraries: pip install pandas numpy scikit-learn")
    exit(1)
...
def extract_features(url):
    ...
    try:
        ...
        # Check URL length
        if len(url_decoded) < 10:
            raise ValueError("URL is too short")
        if len(url_decoded) > 2048:
            raise ValueError("URL is too long")
        # Check for allowed characters after decoding
        if not re.match(r'^[a-zA-Z0-9:/?=&._@%#-+~\[\]\(\)\']+$', url_decoded):
...
def main():
    ...
    print(f"Warning: Test set has only {len(X_test)} samples; at least 10 are recommended for reliable results.")
